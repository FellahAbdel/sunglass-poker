class partie {
  ...
  distribuer() {
    ...
    for (let i = 0; i < this.#players.length; i++) {
      ...
      if (player.getPlayerCards().length < 2) {
        // Vérifier si le joueur a moins de 2 cartes
  ...
  fait7cartes(player) {
    return {
      cards: this.#hiddenCards + player.getPlayerCards(),
    ...
    for (let joueur of listeJoueurCombinaison) {
      ...
      if (paire) {
        ...
        if (valeur > plusHauteValeur) {
          ...
          hautesPaires = [
            {
              main: joueur.main.filter((carte) => carte.numero !== paire),
              id: joueur.id,
            },
